...
tips[66] = '在终端查看日期和显示日历';
code[66] = 'date,cal';
tips[67] = '查看某个磁盘分区可用空间';
code[67] = 'df /dev/sdaX';
tips[68] = '以直观的方式显示系统可用内存';
code[68] = 'free -h';
tips[69] = '将工作目录改为先前的工作目录';
code[69] = 'cd -';
tips[70] = '确定文件类型';
code[70] = 'file <你要确定类型的文件>';
tips[71] = '常用字符类';
code[71] = '匹配任意一个字母或者数字，[:alnum:]； 匹配任意一个字母，[:alpha:]；匹配任意一个数字，[:digit]';
tips[72] = '复制整个目录到另一个目录下';
code[72] = 'cp -r <想要复制的目录> <目标目录>';
tips[73]='合并多个 flv 视频文件';
code[73]='mencoder -oac pcm -ovc copy -idx -o <合并后的文件> <需要合并的文件1> <需要合并的文件2> ... ... ';
tips[74]='将文件移动到一个目录中，只移动目标目录中不存在的文件';
code[74]='mv -u file1 file2';
tips[75]='创建符号链接';
code[75]='ln -s item link';
tips[76]='花括号扩展，一次性创建目录 FileA FileB FileC';
code[76]='mkdir File{A,B,C}';
tips[77]='花括号扩展，一次性创建以日期为文件名的文件夹';
code[77]='mkdir {2013..2053}-0{1..9} {2013..2053}-{10..12}';
tips[78]='命令替换';
code[78]='ls -l $(which cp)';
tips[79]='在终端中移动光标到行首,移动到行末';
code[79]='Ctrl-A，Ctrl-B';
tips[80]='改变文件所有者和群组';
code[80]='chowm [ower][:[group]] file';
tips[81]='以树状模式输出进程列表';
code[81]='pstree';
tips[82]='将文件打包成 iso 映像';
code[82]='mkiso -o <目标 ISO 文件的名称> <要打包的目录>';
tips[83]='修复闪存';
code[83]='先卸载闪存，sudo fsck /dev/sdbX';
tips[84]='从 CD 中创建 iso 镜像';
code[84]='dd if=/dev/cdrom of=XXX.ISO';
tips[85]='从 /etc/passwd 文件中提取每一行的第一个字段';
code[85]='cut -d ':' -f 1 /etc/passwd | head';
tips[86]='合并文本行';
code[86]='paste file1.txt file2.txt';
tips[87]='连接两个文件中具有相同字段的行';
code[87]='join file1 file2';
